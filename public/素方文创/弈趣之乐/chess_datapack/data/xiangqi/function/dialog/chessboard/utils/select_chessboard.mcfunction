...
#坐标映射
execute store result storage pc:chess temp.select.x int 1 run scoreboard players get $x pc_xiangqi_position
execute store result storage pc:chess temp.select.y int 1 run scoreboard players get $y pc_xiangqi_position

function xiangqi:dialog/chessboard/utils/select_chessboard_ with storage pc:chess temp.select
